...
void HapMap::readHapDataVCFXP(string filename, string filename2, HapData& hapData, HapData& hapData2)
{
    ...
    if (NUM_LOCI_MISMATCH)
    {
        ...
        // --------------------------------------------------------
        // Merge-style scan over sorted physical positions
        //
        // Rules:
        //   1. If a position is duplicated in either file, skip all
        //      copies of that position from both files.
        //   2. If a position exists only in one file, skip it there.
